...
//        }
    ...
    private void animation(float dt) {
        var sprites = AssetPool.getSpritesheet("assets/images/spritesheet.png");
        gameObj1.getComponent(SpriteRenderer.class).setSprite(sprites.getSprite(index));
        time += dt;
        if (time > 0.2f) {
            index++;
            time = 0;
        }
        if (index >= 14 || index <= 9) index = 9;
        gameObj1.transform.position.x += 30 * dt;
    }
